...
class Colony:
    ...
    class Ant:
        def __init__(self, row: int, col: int, colony: "Colony", special=False) -> None:
            """Each ant has an initial position and belongs to a colony."""
            self.row = row
            self.col = col
            self.colony = colony
        ...
        def __str__(self) -> str:
            """Format the ant for printing"""
            return "[" + str(self.row) + ", " + str(self.col) + "]"

        def probabilistic_choice(self, positions: list, probabilities: list) -> tuple:
            """Select a position from the given locations based on probability"""
        ...
        def index_probability(self, index: tuple):
            """
             Calculate the probability value for a given position.
        ...
        def get_index_probabilities(self):
            """
            Get the positions and corresponding probability values of the surrounding pixels for an ant.
            :return: List of probability values and list of positions
            """
            numerators = list()
            positions = list()
        ...
        def get_max_probability_pos(self):
            """
            Find the position with the highest probability in the surrounding area
            :return: Position with the highest probability.
            """
        ...
        def deposit_pheromone(self):
            """
            Release pheromones at the position, and if the threshold is reached, move to that position; otherwise, jump randomly
            return：none
            """
            # self.update_memory()
    ...
    def __str__(self) -> str:
        """Print some information about special ants"""
    ...
    def pixel_intensity(self, row: int, col: int) -> float:
        """
       Calculate the intensity/importance of a given pixel, considering its surrounding pixels.
      :param row: The x-coordinate of the pixel
      :param col: The y-coordinate of the pixel
      :return: The normalized maximum intensity
        """
        # (1 / self.i_max)
        return max(
...
# Make sure the script is being run from the command line and that the correct arguments are provided
if __name__ == "__main__":
    # Hardcode the image directory path, i.e., the path to the folder containing the images.
    image_directory = "D:\\sample4chuli\\yuchuli"
    print("Using image directory: " + image_directory)
    #初始化目标内容的变量
    entries = None
    try:
        #Attempt to list all files and folders under the specified target directory
        entries = os.listdir(path=image_directory)
    except FileNotFoundError as FNFE:
        # If the directory does not exist, raise an exception and print the error message to standard error
        print(type(FNFE).__name__ + ": " + image_directory + ": " + str(FNFE), file=sys.stderr)
        exit(2) # Exit the program with error code 2
    # Print the contents of the directory.
    print("Directory [" + image_directory + "]: " + str(entries))
    # Iterate over every file and folder in the directory
    for item in entries:
        # Skip files or folders named 'Intensities' or 'Iterations
        ...
        try:
            ...
            # If an error occurs while reading the image, raise an exception and print the error message to the standard error stream
            print(type(VE).__name__ + ": " + path + " is not a valid image", file=sys.stderr)
            continue
        # Print the image shape or dimensions
        print("[" + item + "] size: " + str(img.shape) + " len: " + str(img.shape[0] * img.shape[1]))
        print(img)# Display the image as a matrix of pixel values
        # Print image data type, max value, min value, and mean value
        print("Image: type: " + str(img.dtype) + " max: " + str(img.max()) + " min: " + str(img.min()) + " mean: " +
              str(img.mean()))
        #  Create a Colony object for image processing
        c = Colony(img_path=path, img=img, ant_count=-1, pheromone_evaporation_constant=0.001,
                   ...
                   alpha=12.5, beta=1.0)
        # Generate the directory path for saving the processed image
        clean_path = os.path.join(image_directory, "Iterations", item.split('.')[0])
        # Call the clean_up method of the Colony object to perform image cleanup and save the result to clean_path
        c.clean_up(dir_path=clean_path)
       #Call the iterate method of the Colony object to perform multiple iterations of processing
        c.iterate(iterations=-1)
        #  Display the processed image
        # plt.imshow(img, cmap='gray')
        # plt.title(f"Processed Image: {item}")
        # plt.axis('off')
        # plt.show()
###################################################################################
